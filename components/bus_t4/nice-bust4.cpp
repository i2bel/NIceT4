...
// === ОБРАБОТКА СОСТОЯНИЯ пакета ===
if (data.size() > 14 && 
    data[4] == 0x00 && data[5] == 0x03 &&  // от привода
    data[2] == 0x00 && data[3] == 0x0A &&  // к OXI
    data[10] == 0x00 && data[11] == 0x19) { // ответ на запрос
    
    uint8_t photo_status = data[14];
    
    // Просто информируем, что пакет прилетел иначе от этого пакета в логе ошибка
	if (photo_status == 0x01) {
        ESP_LOGI(TAG, "status == 0x01");
    }
}
